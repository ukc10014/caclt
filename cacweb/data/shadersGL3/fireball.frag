...
//Putting distance as global, as not sure 'out' qualifier in RenderScene works
//float distance;
...
/*UKC: modified to use dst not distnace to not confuse with global distance*/
/*
vec4 Shade(float dst)
{
	float c1 = saturate(dst*5.0 + 0.5);
	float c2 = saturate(dst*5.0);
	float c3 = saturate(dst*3.4 - 0.5);
...
//float RenderScene(vec3 position) //UKC: because distance is now global
{
...
vec4 March(vec3 rayOrigin, vec3 rayStep)
{
	...
	for(int step = MarchSteps; step >=0  ; --step)
	{
		...
		//displacement = RenderScene(position); //UKC: because distance is now global
