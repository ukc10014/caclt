...
/*UKC: Taken from jmpep on shadertoy.  Open use, downloaded 20/8/20 from https://www.shadertoy.com/view/Xdl3D8*/
...
void main()
{
	...
	uv.y = 1.0 - uv.y;
		...
		vec3 image = texture2D(tex0, vec2(suv.x, suv.y) ).xyz;
			...
			vI = 20.0 * (uv.x * (1.0-uv.x) * uv.y * (1.0-uv.y)); //UKC: first number is brightness (high=bright)
			...
			vI *= pow(5.0 * uv.x * (1.0-uv.x) * uv.y * (1.0-uv.y), 0.4); //UKC: power base is brightness
		#endif
		...
		//UKC 13/11: think it's GRAIN that is problem on iPhones
	        ...
	        gl_FragColor.xyz *= (1.0+(rand(uv+t*.01)-.2)*rand(1.0/uv));		
        #endif		
